    ...
    <xsl:template match="layout:header" xmlns:layout="URN:html-form-layout:v0.1">
      <p class="sectionTitle {@class}" style="{@style}"><span><xsl:apply-templates/></span></p>               
    </xsl:template>
    
    <!-- if ********************************
    <layout:if xmlns:layout="URN:html-form-layout:v0.1">
      text  
    </layout:header>
      *******************************************-->
    <xsl:template match="layout:if" xmlns:layout="URN:html-form-layout:v0.1">  
         ...
         <!--xsl:if test="string-length(@value)>0">
            <xsl:if test="@value=ctx:extract(@test)">
               <xsl:apply-templates/>
            </xsl:if>
         </xsl:if-->  
