<?xml version="1.0" encoding="UTF-8"?>
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:layout="URN:html-form-layout:v0.1" xmlns="http://www.w3.org/1999/xhtml" xmlns:ex2="http://www.oracle.com/XSL/Transform/java/org.fx.xml.XSLTExtension" xmlns:ctx="http://www.oracle.com/XSL/Transform/java/org.fx.xml.XSLDataContext" xmlns:gbu="http://xmlns.oracle.com/MSG_GBU_doc">
<xsl:output method="html" indent="yes" omit-xml-declaration="yes" undeclare-prefixes="yes" exclude-result-prefixes="#all"/>

    <xsl:template match="/">

        <XML>
          <xsl:copy-of select="."/>
        </XML>

        <style>
            div#headerContentArea {
                padding: 0;
            }
#headerContentArea {
    background: #fff;
    padding: 10px 0;
    border-left: 1px solid #D7D8DB;
    border-right: 1px solid #D7D8DB;
    border-bottom: 1px solid #D7D8DB;
    margin-top: -1px;
}
            #tabs-container {
                display: none;
            }
            .lk-header {
                position: relative;
                width: 100%;
                height: 185px;
                line-height: 185px;
                padding: 0;
                background: #0985c5;
                background: -moz-linear-gradient(top,  #0985c5 0%, #41b7dd 99%);
                background: -webkit-linear-gradient(top,  #0985c5 0%,#41b7dd 99%);
                background: linear-gradient(to bottom,  #0985c5 0%,#41b7dd 99%);
                filter: progid:DXImageTransform.Microsoft.gradient( startColorstr='#0985c5', endColorstr='#41b7dd',GradientType=0 );
            }
            .lk-header-link {
                display: inline-block;
                vertical-align: middle;
                position: relative;
                line-height: 1.25;
                margin: 32px 56px 0;
                text-align: center;
                color: #fff;
                text-decoration: none;
            }
            .lk-header-link i {
                display: block;
                width: 90px;
                height: 90px;
                margin: 0 auto 6px;
                border-radius: 50%;
                background-repeat: no-repeat;
                background-position: center center;
            }
            .lk-header-link span {
                font-weight: bold;
                font-size: 19px;
                white-space: nowrap;
            }
            .lk-header-link-requests i {
                background-color: #E06953;
                background-image:url(data:image/png;base64,);
            }
            .lk-header-link-instruction i {
                background-color: #7CD88D;
                background-image:url(data:image/png;base64,);
            }
            .lk-header-link-payment i {
    background-color: #E06953;
    background-image: url(data:image/png;base64,);
}
          .lk-header-link-info i {
    background-color: #6839AB;
    background-color: #EFC62E;
    background-image: url(data:image/png;base64,);
}
            .lk-header-link-appeals i {
    background-color: #6839AB;
    background-image: url(data:image/png;base64,);
}
            .TOP_MENU_element.ugdButton.TOP_MENU.getXslt.left.st_showMenu.MAIN{
                display: none;
            }
            body { background-color: #f0f0f0; margin: 0;     font-family: 'PTSans', Arial, sans-serif; }
h1 {
    color: #5f5f5f;
    text-shadow: 1px 1px 0px #fff;
    font-family: Arial;
    font-weight: bold;
    text-align: center;
    margin-top: 130px;
    font-size: 48px;
}
.pptx, .pdf{
    position: relative;
    margin-left: 40px;
    display: inline-block;
}
.pptx:before, .pdf:before{
    content: '';
    display: block;
    position: absolute;
    width: 38px;
    height: 36px;
    background-size: contain;
    margin-top: -9px;
    margin-left: -40px;
}
.pptx:before{
    background-image: url(/portal_static/images/pptx.svg);
}

.pdf:before{
    background-image: url(/portal_static/images/pdf8.svg);
    background-repeat: no-repeat;
    height: 30px;
    margin-top: -5px;
}
.whiteBlock {
    background: #FFF;
}
.dataBlock {
    border: 1px solid #D7D8DB;
    margin: 9px 5px;
    margin: 20px;
    border-radius: 5px;
    padding: 20px;
}
.dataBlock a:hover {
    color: #c00;
    transition: all .3s ease;
}

.dataBlock a {
    color: #235DC1;
    text-decoration: none;
    transition: all .3s ease;
}

        </style>

<!--div id="headerContentArea">
    <div class="lk-header">
      <a class="lk-header-link lk-header-link-appeals" href="tabInfo.action?tab=SECTION_EXPERTIZA&amp;c=4&amp;b=7EDF6ECC25214253BCB3C9E155610D67&amp;a=2D98307046DF44668AE2E512AAD9985C#tab::id=0/tree::rel=4/filter::id=2D98307046DF44668AE2E512AAD9985C/vitrina::viewId=2D98307046DF44668AE2E512AAD9985C&amp;offset=0&amp;limit=100"><i></i><span>Обращения</span></a>
      <a class="lk-header-link lk-header-link-payment" href="tabInfo.action?tab=SECTION_EXPERTIZA&amp;c=19&amp;b=1D2C177C0D874FB6A6158C58A6BB6843&amp;a=5CB571F9DC1F400092F50291CFBA5FEA#tab::id=0/tree::rel=19/filter::id=5CB571F9DC1F400092F50291CFBA5FEA/vitrina::viewId=5CB571F9DC1F400092F50291CFBA5FEA&amp;offset=0&amp;limit=10"><i></i><span>Оплата</span></a>
      <a class="lk-header-link lk-header-link-instruction" href="/portal/getCardHtml.action?cardId=CARD_ZAYAVITEL_INSTRUCTION">
        <i></i>
        <span>Инструкция</span>
      </a>
      <a class="lk-header-link lk-header-link-info" href="/portal/getCardHtml.action?cardId=CARD_ZAYAVITEL_TECH_HELP">
        <i></i>
        <span>Тех. поддержка</span>
      </a>
    </div>
</div-->

<!--h1>СТРАНИЦА В РАЗРАБОТКЕ</h1-->
<div class="whiteBlock dataBlock">
    <h3><a href="/portal_static/Instrukcija_AIS_Jekspertiza_Proektov_(Administrator).pdf" class="pdf">Инструкция для администраторов</a></h3>
    <h3><a href="/portal_static/Instrukcija_AIS_Jekspertiza_Proektov_(Priem_i_soprovozhdenie).pdf" class="pdf">Инструкция для приема и сопровождения</a></h3>
    <h3><a href="/portal_static/Instrukcija_AIS_Jekspertiza_Proektov_(Dogovorniki).pdf" class="pdf">Инструкция для договорников</a></h3>
    <h3><a href="/portal_static/Instrukcija_AIS_Jekspertiza_Proektov_(Buhgalterija).pdf" class="pdf">Инструкция для бухгалтерии</a></h3>
    <h3><a href="/portal_static/Instrukcija_AIS_Jekspertiza_Proektov_(Vedushhij_jekspert).pdf" class="pdf">Инструкция для ведущего эксперта</a></h3>
    <h3><a href="/portal_static/Instrukcija_AIS_Jekspertiza_Proektov_(Jeksperty).pdf" class="pdf">Инструкция для экспертов</a></h3>

</div>


    </xsl:template>
</xsl:stylesheet>
